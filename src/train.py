from data_processed import feature_engineering_pipeline,load
import mlflow
import os
import pandas as pd
...
class TrainModel:
    ...
    def metrics(self, model):
        ...
        percision = precision_score(self.y_test, y_predict)
        roc = roc_auc_score(self.y_test, y_predict)
        return {
            'accuracy':accuracy,
            'recall': recall,
            'f1': f1,
    ...
    def run(self):
        ...
        self.logging(lr_model, lr_metrics,'Linear Regression', lr_params, 'linear_credit')
        tree_model = self.train_tree(tree_params)
        tree_metrics = self.metrics(tree_model)
        self.logging(tree_model, tree_metrics,'Decsion Tree', tree_params, 'tree_credit')
        gbm_model = self.train_gbm(gbm_params)
        gbm_metrics = self.metrics(gbm_model)
        self.logging(gbm_model, gbm_metrics,'GBM', gbm_params, 'gbm_credit')
